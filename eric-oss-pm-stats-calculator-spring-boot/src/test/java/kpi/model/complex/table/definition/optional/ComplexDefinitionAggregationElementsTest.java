...
class ComplexDefinitionAggregationElementsTest implements OptionalDefinitionAttributeContract<List<ComplexAggregationElement>> {
    @Override
    public OptionalDefinitionAttribute<List<ComplexAggregationElement>> createInstance() {
        return ComplexDefinitionAggregationElements.of(toComplexAggregationElements("table.column", "table.column1 AS column1"));
    }

    @Override
    public TableAttribute<List<ComplexAggregationElement>> createParentInstance() {
        return ComplexTableAggregationElements.of(toComplexAggregationElements("table.column3", "table.column4 AS column4"));
    }
    ...
    @Override
    public Stream<ValueArguments<List<ComplexAggregationElement>>> provideOverridableCases() {
        return Stream.of(
                ValueArguments.of(ComplexDefinitionAggregationElements.of(List.of()), toComplexAggregationElements("table.column"))
        );
    }

    @Override
    public Stream<ValueArguments<List<ComplexAggregationElement>>> provideNonOverridableCases() {
        return Stream.of(
                ValueArguments.of(ComplexDefinitionAggregationElements.of(toComplexAggregationElements("table.column")), null),
                ValueArguments.of(ComplexDefinitionAggregationElements.of(toComplexAggregationElements("table.column")), toComplexAggregationElements("table.column1"))
        );
    }
    ...
    public Stream<DynamicNode> deserializationCustomSuccesses(final TestReporter testReporter) {
        return Stream.of(dynamicTest("When attribute values are missing", () -> {
            ...
            final Attribute<List<ComplexAggregationElement>> attribute = Serialization.deserialize(content, deduceSelfClass());
