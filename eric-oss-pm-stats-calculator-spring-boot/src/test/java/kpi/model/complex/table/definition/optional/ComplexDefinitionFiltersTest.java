...
class ComplexDefinitionFiltersTest implements OptionalDefinitionAttributeContract<List<ComplexFilterElement>> {
    @Override
    public OptionalDefinitionAttribute<List<ComplexFilterElement>> createInstance() {
        return ComplexDefinitionFilters.of(toComplexFilterElements("filter_1", "filter_2"));
    }

    @Override
    public TableAttribute<List<ComplexFilterElement>> createParentInstance() {
    ...
    public String representation() {
        return "filters = [filter_1, filter_2]";
    }

    @Override
    public Class<? extends TableAttribute<List<ComplexFilterElement>>> parentClass() {
        throw new UnsupportedOperationException(String.format("'%s' has no parent value", name()));
    }
    ...
    public void shouldVerifyParentClass() {
        final OptionalDefinitionAttribute<List<ComplexFilterElement>> attribute = createInstance();
    ...
    public Stream<DynamicNode> deserializationCustomSuccesses(final TestReporter testReporter) {
        return Stream.of(dynamicTest("When attribute values are missing", () -> {
            ...
            final Attribute<List<ComplexFilterElement>> attribute = Serialization.deserialize(content, deduceSelfClass());
    ...
    public Stream<DynamicNode> deserializationCustomFailures(final TestReporter testReporter) {
        ...
        final DynamicNode whenAnyValueHasInvalidParameter = dynamicTest("When any attribute value contains not supported parameter token", () -> {
            final String content = "{ \"" + name() + "\": [\"${param.invalid_param}\"] }";
            Assertions.assertThatThrownBy(() -> Serialization.deserialize(content, deduceSelfClass()))
                    .isInstanceOf(ValueInstantiationException.class)
                    .hasRootCauseInstanceOf(IllegalArgumentException.class)
                    .getRootCause()
                    .hasMessageContaining("The attribute 'filter' only support '${param.start_date_time}' or '${param.end_date_time}' as parameter");
            testReporter.publishEntry(name(), content);
        });
        ...
        final DynamicNode whenAttributeValueContainsFrom =
                dynamicTest("When attribute value contains 'FROM'", () -> {
                    final String value = "kpi_db://kpi_simple_60.aggregation_begin_time BETWEEN (date_trunc('hour', " +
                            "TO_TIMESTAMP('${param.start_date_time}')) - interval 1 day) and date_trunc('hour', TO_TIMESTAMP('${param.end_date_time}')) "
                            + KpiCalculatorConstants.SQL_FROM;
                    final String content = "{ \"" + name() + "\": [\"" + value + "\"] }";
                    Assertions.assertThatThrownBy(() -> Serialization.deserialize(content, deduceSelfClass()))
                            .isInstanceOf(ValueInstantiationException.class)
                            .hasRootCauseInstanceOf(IllegalArgumentException.class)
        ...
        final DynamicNode whenAttributeValueContainsWhere =
                dynamicTest("When attribute value contains 'WHERE'", () -> {
                    final String value = "kpi_db://kpi_simple_60.aggregation_begin_time BETWEEN (date_trunc('hour', " +
                            "TO_TIMESTAMP('${param.start_date_time}')) - interval 1 day) and date_trunc('hour', TO_TIMESTAMP('${param.end_date_time}')) "
                            + KpiCalculatorConstants.SQL_WHERE;
                    final String content = "{ \"" + name() + "\": [\"" + value + "\"] }";
                    Assertions.assertThatThrownBy(() -> Serialization.deserialize(content, deduceSelfClass()))
                            .isInstanceOf(ValueInstantiationException.class)
                            .hasRootCauseInstanceOf(IllegalArgumentException.class)
        ...
        return Stream.concat(
                ...
                Stream.of(whenAnyValueIsNull, whenAnyValueHasInvalidParameter, whenAnyValueDoesNotMatchPattern, whenAttributeValueContainsFrom, whenAttributeValueContainsWhere)
        );
    }
}
