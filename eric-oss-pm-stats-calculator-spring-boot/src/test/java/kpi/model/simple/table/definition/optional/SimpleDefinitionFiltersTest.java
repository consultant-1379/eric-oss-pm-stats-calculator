...
class SimpleDefinitionFiltersTest implements OptionalDefinitionAttributeContract<List<SimpleFilterElement>> {
    @Override
    public OptionalDefinitionAttribute<List<SimpleFilterElement>> createInstance() {
        return SimpleDefinitionFilters.of(toFilterElements("filter_1", "filter_2://source"));
    }

    @Override
    public TableAttribute<List<SimpleFilterElement>> createParentInstance() {
    ...
    @Override
    public Class<? extends TableAttribute<List<SimpleFilterElement>>> parentClass() {
        throw new UnsupportedOperationException(String.format("'%s' has no parent value", name()));
    }
    ...
    public void shouldVerifyParentClass() {
        final OptionalDefinitionAttribute<List<SimpleFilterElement>> attribute = createInstance();
    ...
    public Stream<DynamicNode> deserializationCustomSuccesses(final TestReporter testReporter) {
        return Stream.of(dynamicTest("When attribute values are missing", () -> {
            ...
            final Attribute<List<SimpleFilterElement>> attribute = Serialization.deserialize(content, deduceSelfClass());
    ...
    public Stream<DynamicNode> deserializationCustomFailures(final TestReporter testReporter) {
        ...
        final DynamicNode whenAttributeValueContainsFrom =
            dynamicTest("When attribute value contains 'FROM'", () -> {
                final String value = "nodeFDN = 'node1' " + KpiCalculatorConstants.SQL_FROM;
                final String content = "{ \"" + name() + "\": [\"" + value + "\"] }";
                Assertions.assertThatThrownBy(() -> Serialization.deserialize(content, deduceSelfClass()))
                        .isInstanceOf(ValueInstantiationException.class)
                        .hasRootCauseInstanceOf(IllegalArgumentException.class)
        ...
        final DynamicNode whenAttributeValueContainsWhere =
            dynamicTest("When attribute value contains 'WHERE'", () -> {
                final String value = "nodeFDN = 'node1' " + KpiCalculatorConstants.SQL_WHERE;
                final String content = "{ \"" + name() + "\": [\"" + value + "\"] }";
                Assertions.assertThatThrownBy(() -> Serialization.deserialize(content, deduceSelfClass()))
                        .isInstanceOf(ValueInstantiationException.class)
                        .hasRootCauseInstanceOf(IllegalArgumentException.class)
        ...
        return Stream.concat(
                ...
                Stream.of(whenAnyValueIsNull, whenAnyValueIsParameterized, whenAnyValueDoesNotMatchPattern, whenAttributeValueContainsFrom, whenAttributeValueContainsWhere)
        );
    }
}
