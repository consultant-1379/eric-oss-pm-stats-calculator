/*******************************************************************************
 * COPYRIGHT Ericsson 2023
 *
 * The copyright to the computer program(s) herein is the property of
 * Ericsson Inc. The programs may be used and/or copied only with written
 * permission from Ericsson Inc. or in accordance with the terms and
 * conditions stipulated in the agreement/contract under which the
 * program(s) have been supplied.
 ******************************************************************************/

package com.ericsson.oss.air.pm.stats.service.api;

import java.time.LocalDateTime;
import java.util.List;
import java.util.Map;
import java.util.UUID;

import com.ericsson.oss.air.pm.stats.model.entity.CalculationReliability;

public interface CalculationReliabilityService {
    Map<String, LocalDateTime> findReliabilityThresholdByCalculationId(UUID calculationId);

    Map<String, LocalDateTime> findStartTimeByCalculationId(UUID calculationId);

    Map<String, LocalDateTime> findMaxReliabilityThresholdByKpiName();

    void save(List<CalculationReliability> calculationReliabilities);
}
