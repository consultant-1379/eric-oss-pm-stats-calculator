...
/**
 * Abstract {@link UserDefinedAggregateFunction} class that holds common attributes and methods for concrete {@link ArrayIndexSumUdaf} and
 * {@link PercentileIndexUdaf} classes
 */
@Slf4j
@SuppressWarnings({"squid:S1874", "deprecation"})
abstract class AbstractArrayUdaf extends UserDefinedAggregateFunction {
    ...
    protected AbstractArrayUdaf(final DataType returnDataType) {
        super();
        final List<StructField> inputFields = new ArrayList<>(1);
        inputFields.add(DataTypes.createStructField("inputArray", DataTypes.createArrayType(DataTypes.IntegerType), true));
        inputDataType = DataTypes.createStructType(inputFields);

        final List<StructField> bufferFields = new ArrayList<>(1);
        bufferFields.add(DataTypes.createStructField("bufferArray", DataTypes.createArrayType(DataTypes.IntegerType), true));
    ...
    public void update(final MutableAggregationBuffer buffer, final Row input) {
        ...
        if (!input.isNullAt(0)) {
            if (buffer.isNullAt(0)) {
                ...
                final List<Integer> newValue = new ArrayList<>(convertNullValuesToZero(input.getList(0)));
                final List<Integer> iniVal = new ArrayList<>(convertNullValuesToZero(buffer.getList(0)));
    ...
    private static List<Integer> convertNullValuesToZero(final List<Integer> input) {
        final List<Integer> nonNullValues = new ArrayList<>(input.size());

        for (final Integer i : input) {
            final int valueToAdd = i == null ? 0 : i;
    ...
    public void merge(final MutableAggregationBuffer buffer1, final Row buffer2) {
        ...
        if (!buffer2.isNullAt(0)) {
            if (buffer1.isNullAt(0)) {
                ...
                // Otherwise, we add the input value to the buffer value.
                final List<Integer> newValue = new ArrayList<>(buffer2.getList(0));
                final List<Integer> iniVal = new ArrayList<>(buffer1.getList(0));
    ...
    private static Integer convertNullToZero(final Integer value) {
