...
abstract class AbstractLongArrayUdaf extends UserDefinedAggregateFunction {
    ...
    protected AbstractLongArrayUdaf(final DataType returnDataType) {
        super();
        final List<StructField> inputFields = new ArrayList<>(1);
        inputFields.add(DataTypes.createStructField("inputArray", DataTypes.createArrayType(DataTypes.LongType), true));
        inputDataType = DataTypes.createStructType(inputFields);

        final List<StructField> bufferFields = new ArrayList<>(1);
        bufferFields.add(DataTypes.createStructField("bufferArray", DataTypes.createArrayType(DataTypes.LongType), true));
    ...
    public void update(final MutableAggregationBuffer buffer, final Row input) {
        ...
        if (!input.isNullAt(0)) {
            if (buffer.isNullAt(0)) {
                ...
                final List<Long> newValue = new ArrayList<>(convertNullValuesToZero(input.getList(0)));
                final List<Long> iniVal = new ArrayList<>(convertNullValuesToZero(buffer.getList(0)));
    ...
    private static List<Long> convertNullValuesToZero(final List<Long> input) {
        final List<Long> nonNullValues = new ArrayList<>(input.size());

        for (final Long value : input) {
            final long valueToAdd = value == null ? 0 : value;
    ...
    public void merge(final MutableAggregationBuffer buffer1, final Row buffer2) {
        ...
        if (!buffer2.isNullAt(0)) {
            if (buffer1.isNullAt(0)) {
                ...
                // Otherwise, we add the input value to the buffer value.
                final List<Long> newValue = new ArrayList<>(buffer2.getList(0));
                final List<Long> iniVal = new ArrayList<>(buffer1.getList(0));
    ...
    private static Long convertNullToZero(final Long value) {
