...
@Slf4j
@ApplicationScoped
public class ReliabilityThresholdTemporalAdjuster {

    public LocalDateTime calculate(@NonNull final ReadinessLog readinessLog, @NonNull final KpiDefinitionEntity kpiDefinition) {
        final LocalDateTime latestCollectedData = readinessLog.getLatestCollectedData();
        final LocalDateTime reliabilityOffsetAppliedTime = latestCollectedData.minusMinutes(kpiDefinition.dataReliabilityOffset());
        final int aggregationPeriod = kpiDefinition.aggregationPeriod();

        return aggregationPeriod == DEFAULT_AGGREGATION_PERIOD_INT
                ? reliabilityOffsetAppliedTime
                : LocalDateTimeTruncates.truncateToAggregationPeriod(reliabilityOffsetAppliedTime, aggregationPeriod);
    }
}
