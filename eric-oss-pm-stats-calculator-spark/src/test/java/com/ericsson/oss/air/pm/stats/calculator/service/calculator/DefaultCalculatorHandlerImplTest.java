...
class DefaultCalculatorHandlerImplTest {
    ...
    @InjectMocks DefaultCalculatorHandlerImpl objectUnderTest;
    ...
    class CalculatorInstantiation {
        @Mock
        Collection<KpiDefinition> kpiDefinitionsMock;

        @Test
        void shouldInstantiateAppropriateCalculator() {
            try (final MockedConstruction<KpiCalculatorDefaultFilter> mockedConstruction = mockConstruction(KpiCalculatorDefaultFilter.class)) {
                ...
                Assertions.assertThat(actual).isExactlyInstanceOf(KpiCalculatorDefaultFilter.class);
    ...
    static Stream<Arguments> provideSupportData() {
        return Stream.of(
                Arguments.of(FilterType.DEFAULT, true),
                Arguments.of(FilterType.CUSTOM, false)
        );
    }
}