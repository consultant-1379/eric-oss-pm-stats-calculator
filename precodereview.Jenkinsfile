...
final String bob = new Bob().command()
...
pipeline {
    ...
    environment {
        ...
        IMAGE_AND_PACKAGE_ENABLED = "false"
        // FOSSA
        ...
        MUNIN_TOKEN = credentials('munin-token-manual')
        FOSSA_ENABLED = "true"
    ...
    stages {
            ...
            steps {
                ...
                script {
                    ...
                    if(env.MUNIN_UPDATE_ENABLED == "true") {
                        sh "${bob} -r ruleset2.0.mimer.yaml mimer-init"
                    }
                }
            }
        }
        ...
        stage('Lint') {
            steps {
                parallel(
                    //  "lint checkstyle": {    //  TODO: Enable checkstyle once we can fail on the result
                    //       sh "${bob} lint:checkstyle"
                    //       archiveArtifacts([
                    //              allowEmptyArchive: true,
                    //              artifacts: 'build/checkstyle.log'
                    //       ])
                    //  },
                    "lint markdown": {
            ...
            post {
                always {
                    archiveArtifacts([
                            allowEmptyArchive: true,
                            artifacts: '**/*bth-linter-output.html, \
                    ...
                    "Open API Spec": {
                        ...
                        script {
                            ...
                            if (val.trim().equals("true")) {
                                ...
                                archiveArtifacts allowEmptyArchive: true, artifacts: "${OPEN_API_SPEC_DIRECTORY}/rest_conversion_log.txt, ${OPEN_API_SPEC_DIRECTORY}/rest2html*.zip"
        ...
        stage('Test') {
            steps {
                sh "${bob} test-without-integration-jar"
            }
        }
        ...
        stage('Image') {
            when {
                expression { env.IMAGE_AND_PACKAGE_ENABLED == "true" }
            }
            steps {
        ...
        stage('Package') {
            ...
            steps {
                sh "${bob} package-without-integration-jar"
                sh "${bob} package-jars"
            }
        }
        ...
        stage('K8S Resource Lock') {
            ...
            stages {
                stage('Helm Install') {
                    when {
                        expression { env.K8S_TEST == 'true' }  // DISABLED STAGE
                    }
                    steps {
        ...
        stage('Generate Vulnerability report V2.0') {
            steps {
                ...
                sh "${bob} generate-VA-report-V2:analyze-va-result"
            }
        }
        ...
        stage('Upload EriDoc Documentation') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'eridoc-user', usernameVariable: 'ERIDOC_USERNAME', passwordVariable: 'ERIDOC_PASSWORD')]) {
                    // Upload EriDoc Documentation Dryrun
                    sh "${bob} generate-docs:markdown-to-pdf"
                    script {
                        echo "Upload EriDoc Documentation Dryrun"
                        sh "${bob} eridoc-upload-documents:eridoc-upload-dryrun"
                    }
                }
            }
        }
...
def addHelmDRWarningIcon() {
    final String designRuleCheckResult = readFile '.bob/var.helm-chart-check-report-warnings'
    if (designRuleCheckResult.trim() == 'true') {
        echo '''WARNING: One or more Helm Design Rules have a WARNING state.
                Review the Archived Helm Design Rule Check Report: design-rule-check-report.html'''
        manager.addWarningBadge('''One or more Helm Design Rules have a WARNING state.
                                   Review the Archived Helm Design Rule Check Report: design-rule-check-report.html''')
    } else {
...
/**
 * @see <a href="https://gerrit-gamma.gic.ericsson.se/plugins/gitiles/adp-cicd/bob/+/HEAD/USER_GUIDE_2.0.md#Getting-started">Bob Documentation</a>
 */
@Builder(builderStrategy = SimpleStrategy, prefix = '')
class Bob {
    def bobImage = 'armdocker.rnd.ericsson.se/proj-adp-cicd-drop/bob.2.0:1.7.0-55'
    ...
    String command() {
        final def command = """\
            ...
            |${envVars.collect({ final entry -> "-e ${entry.key}=\"${entry.value}\"" }).join(' ')}
            |\$(for group in \$(id -G); do printf ' --group-add %s' "\$group"; done)
            |--group-add \$(stat -c '%g' /var/run/docker.sock)
            |${bobImage}
            |"""
        return command.stripMargin()
                .replace('\n', ' ')
                .replaceAll('[ ]+', ' ')
    }
}
